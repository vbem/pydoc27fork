...
def serve(host, port, callback=None, completer=None):
...
pydoc</strong> by Ka-Ping Yee &lt;ping@lfw.org&gt;</font>'''
    ...
    try:
        try:
            DocServer(host, port, callback).serve_until_quit()
...
def gui():
    ...
    class GUI:
        def __init__(self, window, host='localhost', port=7464):
            self.window = window
            self.server = None
            self.scanner = None
            ...
            import threading
            threading.Thread(
                target=serve, args=(host, port, self.ready, self.quit)).start()
...
def cli():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'gk:h:p:w')
        writing = 0
...
Introduction of this fork: https://github.com/vbem/pydoc27fork
...
%s -h <hostname>
    Specify the HTTP server hostname listening. This option works with `-p`
    and defaults to `localhost`.
    
%s -p <port>
    Start an HTTP server on the given port on specified hostname.  Port
